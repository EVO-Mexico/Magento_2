...
namespace EVOMexico\Payment\Helper\sdk\lib;

use EVOMexico\Payment\Helper\sdk\lib\Exception\EndpointException;
use EVOMexico\Payment\Helper\sdk\lib\Exception\NetworkException;
use EVOMexico\Payment\Helper\sdk\lib\Exception\ParametersException;
use EVOMexico\Payment\Helper\sdk\lib\Exception\PaymentsExceptionParamNotSet;
use EVOMexico\Payment\Helper\sdk\lib\Exception\PaymentsExceptionProcessDataNotSet;
use EVOMexico\Payment\Helper\sdk\lib\Exception\ProcessException;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestAuth;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestAvailable;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestCapture;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestPurchase;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestStatusCheck;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestTokenize;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestVerify;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestVoid;
use EVOMexico\Payment\Helper\sdk\lib\Request\RequestRefund;
use EVOMexico\Payment\Helper\sdk\lib\Response\ResponseError;
use EVOMexico\Payment\Helper\sdk\lib\Response\ResponseSuccess;
...
class Request extends Executable
{
            ...
            foreach ($this->_params as $key => $value) {
                if ($value["type"] == "mandatory") {
                    if (!isset($this->_data[$key])) {
                        $ex = new ParametersException($key, null, isset($ex) ? $ex : null);
                    }
                } else {
    ...
    protected function _exec_post($url, $data, $callback = null)
    {
        ...
        if ((!is_null($callback)) && (is_callable($callback))) {
            ...
            if (!isset($rawResult)) {
                throw new NetworkException('No [result] field in the response, please check.',
                    curl_errno($ch));
            }
            ...
            if ($rawResult === "failure") {
                return new ResponseError($response, $response["errors"]);
            }
