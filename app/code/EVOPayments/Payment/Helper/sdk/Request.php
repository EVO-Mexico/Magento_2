...
abstract class Request extends Configurable implements Executable {
    ...
    protected function _exec_post($url, $data, $callback = null) {
        if (empty($url)) {
            throw new PaymentsExceptionConfigurationEndpointNotSet('EVOPayments not configured');
        }
        if ((empty($data)) or ( !is_array($data)) or ( count($data) == 0)) {
            throw new PaymentsExceptionProcessDataNotSet('EVOPayments not configured');
        }
        $ch = curl_init($url);
