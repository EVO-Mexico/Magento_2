...
namespace EVOPayments\Payment\Model;

require_once(__DIR__ . '/../Helper/sdk/Configurable.php');
require_once(__DIR__ . '/../Helper/sdk/RequestFactory.php');

use EVOPayments\Payment\Model\Config\Source\Brand;
use EVOPayments\sdk\Configurable;
use EVOPayments\sdk\RequestFactory;
use EVOPayments\Payment\Helper\Helper;
use EVOPayments\Payment\Model\Config\Source\NewOrderPaymentActions;
use EVOPayments\Payment\Model\Config\Source\DisplayMode;
...
{
    const METHOD_CODE = 'evopayments_payment';
    ...
    /**
     * @var \EVOPayments\Payment\Helper\Helper
     */
    private $_helper;
    /**
    ...
    public function getCheckoutRedirectUrl()
    {
       ...
       if ($displayMode === DisplayMode::DISPLAY_MODE_IFRAME) {
           $redirectUrl = 'evopayments/hosted/iframe';
        } else {
            $redirectUrl = 'evopayments/hosted/redirect';
        }
    ...
    private function getTokenHostedData($apiOperation)
    {
        ...
        $sessionTokenData = array(
            ...
            "channel" => Configurable::CHANNEL_ECOM,
            "amount" => $amount,
            ...
            "customerBillingAddressPostalCode" => $customerAddressPostalCode,
            "customerBillingAddressCity" => $customerAddressCity,
            "customerBillingAddressCountry" => $customerAddressCountry,
            "customerBillingAddressStreet" => $customerAddressStreet,
            "customerFirstName" => $customerFirstName,
            "customerLastName" => $customerLastName

        );
        $paymentMethodID = trim($this->_helper->getConfigData('payment_method'));
        if ($paymentMethodID == '500') {
            $sessionTokenData["paymentSolutionId"] = $paymentMethodID;
        }
        // brand id
        ...
        if ($order->getCustomerId() == '') {
            $sessionTokenData['firstTimeTransaction '] = 'true';
        }
        // language
 ...
 /*           foreach($tokenData as $key => $value) {
    ...
    private function toAPIOperation($paymentAction) {
        switch ($paymentAction) {
            case NewOrderPaymentActions::PAYMENT_ACTION_AUTH: {
                return Configurable::ACTION_AUTH;
            }
            case NewOrderPaymentActions::PAYMENT_ACTION_SALE: {
                return Configurable::ACTION_PURCHASE;
            }
            default: {
    ...
    public function authorize(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $sessionTokenData = $this->getTokenDirectData(Configurable::ACTION_AUTH, $data);
        ...
        $params = $this->getAPIParametersForDirect(Configurable::ACTION_CAPTURE, $data);
        $params['token'] = $token->token;
        $result = $this->_helper->executeGatewayTransaction(Configurable::ACTION_AUTH, $params);
    ...
    private function getTokenDirectData($apiOperation, $data = array())
    {
        switch ($apiOperation) {
            ...
            case Configurable::ACTION_VERIFY:
                {
                    return $this->getTokenDirectDataAuthPurchaseVerify($apiOperation, $data);
                }
    ...
    private function getTokenDirectDataAuthPurchaseVerify($apiOperation, $data = array())
    {
        $payment = $this->getInfoInstance();
        $order = $payment->getOrder();
        // allow origin url
    ...
    public function capture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $sessionTokenData = $this->getTokenDirectData(Configurable::ACTION_CAPTURE, $data);
        ...
        $result = $this->_helper->executeGatewayTransaction(Configurable::ACTION_CAPTURE, $params);
        if($result->result === 'success') {
    ...
    public function refund(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        $sessionTokenData = $this->getTokenDirectData(Configurable::ACTION_REFUND, $data);
/*
        $token = $this->getToken($sessionTokenData);
        if ($token->result != 'success' ) {
            throw new \Magento\Framework\Validator\Exception(__(json_encode($token->errors)));
...
*/
        $params = $this->getAPIParametersForDirect(Configurable::ACTION_REFUND, $data);
        //$params['token'] = $token->token;
        $params = array_merge($sessionTokenData, $params);
        $result = $this->_helper->executeGatewayTransaction(Configurable::ACTION_REFUND, $params);
        if($result->result === 'success') {
            $payment->setTransactionId($result->originalMerchantTxId)
                ->setTransactionAdditionalInfo(\Magento\Sales\Model\Order\Payment\Transaction::RAW_DETAILS, json_encode($result));
            $order = $payment->getOrder();
    ...
    public function void(\Magento\Payment\Model\InfoInterface $payment)
    {
        ...
        $sessionTokenData = $this->getTokenDirectData(Configurable::ACTION_VOID);
/*
        $token = $this->getToken($sessionTokenData);
        if ($token->result != 'success' ) {
            throw new \Magento\Framework\Validator\Exception(__(json_encode($token->errors)));
...
*/
        $params = $this->getAPIParametersForDirect(Configurable::ACTION_VOID);
        //$params['token'] = $token->token;
        $params = array_merge($sessionTokenData, $params);
        $result = $this->_helper->executeGatewayTransaction(Configurable::ACTION_VOID, $params);
        if($result->result === 'success') {
            $payment->setTransactionId($result->originalMerchantTxId)
                ->setTransactionAdditionalInfo(\Magento\Sales\Model\Order\Payment\Transaction::RAW_DETAILS, json_encode($result));
            $order = $payment->getOrder();
